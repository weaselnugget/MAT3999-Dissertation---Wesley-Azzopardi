...
def coupled_quad(xi, x, y):
    a1,b1,c1,d1,e1,f1,a2,b2,c2,d2,e2,f2 = xi # Function for Coupled Quadratic
    x_next = a1 + b1*x + c1*y + d1*x**2 + e1*y**2 + f1*x*y
    y_next = a2 + b2*x + c2*y + d2*x**2 + e2*y**2 + f2*x*y
...
kf = KalmanFilter(
    ...
    transition_covariance=1e-2*np.eye(12),
    observation_covariance=1e-4*np.eye(2),
    initial_state_mean=np.ones(12),
...
for t in range(len(Y_train)):
    x_1_pred_train[t], x_2_pred_train[t] = coupled_quad(xi_train[t], x_1_state, x_2_state)
    x_1_state, x_2_state = Y_train[t]
...
for t in range(len(x_1_std)-1):
    x_1_kf[t+1], x_2_kf[t+1] = coupled_quad(xi_full[t], x_1_kf[t], x_2_kf[t])
...
Phi_full = np.column_stack([
    np.ones_like(x_1_prev_full),
    x_1_prev_full,
    x_2_prev_full,
    x_1_prev_full**2,
    x_2_prev_full**2,
    x_1_prev_full*x_2_prev_full])

T_full = len(Phi_full)
C_full = np.zeros((T_full, 2, 12))
C_full[:,0,0:6] = Phi_full
C_full[:,1,6:12] = Phi_full
...
for t in range(len(Y_test)):
    ...
    x_1_pred_test[t], x_2_pred_test[t] = coupled_quad(
        xi_pred,
        x_1_state,
        x_2_state)
...
for t in range(split, len(y_1_full)-(max_steps)):
    ...
    x_1_temp, x_2_temp = coupled_quad(xi_pred, x_1_state, x_2_state)
...
if (max_steps > 1): # Rest of the Steps (Blind-Search)
    for horizon in range(2, max_steps):
        ...
        for t in range(split-(horizon-1), len(y_1_full[:-(max_steps-horizon)])+1-horizon):
            ...
            for _ in range(horizon):
                x_1_temp, x_2_temp = coupled_quad(xi_pred, x_1_temp, x_2_temp)
...
plt.title("Quadratic EMKF Reconstruction on Training Phase of " + column_name_1)
...
plt.title("Quadratic EMKF Reconstruction on Training Phase of " + column_name_2)
...
plt.plot(dates[split:split+len(x_1_real[split:])+(max_steps-1)],
         x_1_real_all[split:],
         color='blue',
         label="Actual")
