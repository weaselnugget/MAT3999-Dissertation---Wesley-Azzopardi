...
def vary_a(): # Fixing b, Varying a
    ...
    plt.scatter(as_plot, ys, s=0.0001, color="red", alpha=1, label="y")
    plt.title(f"Henon Bifurcation Plot for a, b={b:.2f}")
    ...
    plt.axvline(1.0703, color='g', linestyle='--')
    plt.axvline(1.4, color='k', linestyle='-')
    plt.xlabel("a")
    plt.show()
    ...
    for a in a_values:
        ...
        for _ in range(n_iter):
            L = np.array([[-2*a*x, 1],[b, 0]]) # Jacobian
...
def vary_b(): # Fixing a, Varying b
    a = 1.4
    b_values = np.linspace(-0.5, 0.5, 5000)
    ...
    plt.scatter(bs_plot, ys, s=0.001, color="red", alpha=1, label="y")
    plt.title(f"Henon Bifurcation Plot for b, a={a:.2f}")
