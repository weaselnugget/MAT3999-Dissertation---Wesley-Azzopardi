...
def ukf_henon_estimation(a_true, a_guess, b_true, b_guess, rng, n_steps=100, Q_error=1e-5, x0_true=0.1, x0_hat=0.1, y0_true = 0.1, y0_hat = 0.1, meas_noise=None, alpha=1e-3, beta=2, kappa=0):
    ...
    for k in range(1, n_steps): # UKF Loop
        ...
        y_pred = np.sum(Wm[:, None] * Y_sigma, axis=0) # Measurement of Mean

        S = np.zeros((m, m)) # Measurement of Variance Covariance
        for i in range(2*n + 1):
...
a_true_list = [0, 0.284, 0.568, 0.852, 1.136, 1.42] # Run Experiment
b_true_list = [-0.3, -0.18, -0.06, 0.06, 0.18, 0.3]
...
for m, a_true in enumerate(a_true_list): # Create Grid
    for n, b_true in enumerate(b_true_list):
        for o, Q_error in enumerate(Q_list):
            a_final, b_final, runtime = ukf_henon_estimation(a_true=a_true, b_true=b_true, a_guess=a_guess, b_guess=b_guess, rng=rng, n_steps=n_steps, Q_error=Q_error)
            times[m, n, o] = runtime
