...
x_train = x_raw[:split+1] # Standardise using Training
xmin = x_train.min()
xmax = x_train.max()
scale = xmax - xmin
x_std = (x_raw - xmin) / scale

def logistic_map(xi1, xi2, x): # Function for Logistic
    return xi1*x + xi2*x**2

y = x_std[1:]
x_prev = x_std[:-1]
C = np.zeros((len(y), 1, 2)) # Measurement Matrix
C[:,0,0] = x_prev
C[:,0,1] = x_prev**2 # Linear-in-parameters SSM
y_train = y[:split-1]
y_test  = y[split-1:]
C_train = C[:split-1]
C_test  = C[split-1:]
...
for t in range(len(y_train)):
    r = xi_train[t]
    x_pred_train[t] = logistic_map(r[0], r[1], x_state)
    x_state = y_train[t]  # move forward using actual

x_pred_train_real = scale * x_pred_train   + xmin
y_train_real = scale * y_train   + xmin # Convert to real from Standardised
rmse_train = np.sqrt(np.mean((y_train_real - x_pred_train_real)**2))
print("Training RMSE:", rmse_train)

kf_full = KalmanFilter( # Full Reconstruction
    ...
    initial_state_mean=xi_train[0],
    initial_state_covariance=P_train[0])
...
for t in range(len(x_std)-1):
    r_t = xi_full[t]
    x_kf[t+1] = logistic_map(r_t[0], r_t[1], x_kf[t])

x_kf_real   = scale * x_kf   + xmin
x_real = x_raw # Rescale Reconstruction
...
x_std_ahead = (x_ahead - xmin) / scale
y_full = np.concatenate([y, x_std_ahead[:max_steps]])
x_prev_full = np.concatenate([x_std[:-1], x_std[-1:], x_std_ahead[:max_steps-1]])
C_full = np.zeros((len(y_full), 1, 2)) # Measurement Matrix
C_full[:,0,0] = x_prev_full
C_full[:,0,1] = x_prev_full**2 # Linear-in-parameters SSM
...
for t in range(len(y_test)):
    ...
    r = xi_pred
    x_pred_test[t] = logistic_map(r[0], r[1], x_state)
...
x_pred_test_real = scale * x_pred_test   + xmin
y_test_real = scale * y_test   + xmin
rmse_test = np.sqrt(np.mean((y_test_real - x_pred_test_real)**2))
print("Test RMSE (1-step filtering):", rmse_test)
...
for t in range(split, len(y_full)-(max_steps)):
    ...
    r = xi_pred
    x_temp = x_state
    x_temp = logistic_map(r[0], r[1], x_temp)
...
x_fore_real = scale * x_fore_std   + xmin
...
plt.title("Logistic EMKF Reconstruction on Training Phase of " + column_name)
...
length = len(dates[split:split+len(x_real[split:])+(max_steps-1)])
